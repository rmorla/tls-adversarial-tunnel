...
class Docker ( Host ):
    ...
    def startShell( self, *args, **kwargs ):
        ...
        if self.dnameprefix is not None:
            container_name = '%s.%s' % ( self.dnameprefix, self.name )
        else:
            container_name = self.container_name
        
        cmd = [ 'docker', 'exec', '-it',  container_name, 'env', 'PS1=' + chr( 127 ),
    ...
    def popen( self, *args, **kwargs ):
        ...
        mncmd = ["docker", "exec", "-t", container_name]
...
class DockerRunning ( Docker ):
    def __init__(self, name, container_name=None, **kwargs):
        self.container_name = container_name
        self.dnameprefix = None
        self.dc = None  # pointer to the dict containing 'Id' and 'Warnings' keys of the container
        self.dcinfo = None
        self.did = None # Id of running container
        ...
        self.dimage = None
        # fetch information about new container
        self.dcinfo = self.dcli.inspect_container(self.container_name)
        if self.dcinfo is not None and 'Id' in self.dcinfo:
            self.did = self.dcinfo.get("Id")
        else:
